...
/**
 * TinyAssistant — front-only FAQ assistant (English UI)
 * - zero backend, zero keys
 * - preset templates / FAQ
 * - typing simulation
 * - flexible matching (exact, contains, token-overlap, keywords)
 * - per-session question limit (sessionStorage)
 *
 * Exports:
 *   export const TinyAssistant
 *   export default TinyAssistant
 *
 * Drop-in: import TinyAssistant from './components/TinyAssistant';
 */
...
export const TinyAssistant: React.FC = () => {
  ...
  const sendUser = (text: string) => {
    ...
    if (!canAsk) {
      pushMessage({
        id: uid("sys_"),
        role: "assistant",
        text: `Question limit reached (${MAX_QUESTIONS_PER_SESSION}). Press "Reset" to start a new session.`,
        ts: Date.now(),
      });
      return;
    }

    const userMsg: Message = { id: uid("u_"), role: "user", text: text.trim(), ts: Date.now() };
    pushMessage(userMsg);
    setAskedCount((c) => c + 1);
    setInput("");
  ...
  const simulateTypingReply = (fullText: string) => {
    ...
    const speed = 20 + Math.floor(Math.random() * 40);
    let i = 0;
    typingIntervalRef.current = window.setInterval(() => {
      i += 1;
      setMessages((cur) =>
        cur.map((m) => (m.id === id ? { ...m, text: fullText.slice(0, i) } : m))
      );
